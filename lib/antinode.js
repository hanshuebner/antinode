...
function serve_static_file(path, req, resp) {
    ...
    function check_path (path) {
        log.info('check_path', path);
        fs.stat(path, function (err, stats) {
            if (err) {
                // ENOENT is normal on 'file not found', ENOTDIR
                // occurs when extra pathname components follow the
                // name of the .sjs script.
                if (err.errno != constants.ENOENT
                   && err.errno != constants.ENOTDIR) { 
                    // any other error is abnormal - log it
                    log.error("fs.stat(",path,") failed: ", err);
                }
                match = path.match("(.*)/");
                if (match) {
                    return check_path(match[1]);
                } else {
            ...
            if (!stats.isFile()) {
                ...
                if (path.match(/\.sjs$/)) {
                    req.pathArgs = full_path.replace(path + "/", "");
